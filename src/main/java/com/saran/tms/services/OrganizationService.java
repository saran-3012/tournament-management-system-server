...
public class OrganizationService {
	...
	public static List<Model> findOrganizationById(Map<String, String> params) throws ResponseException {
		...
		List<Model> organizationDetails = orgDao.findOneWithJoin(
												...
												Arrays.asList(
													new JoinEntry(TableNames.ORGANIZATIONS, TableNames.USERS, "organization_id", "organization_id", JoinTypes.JOIN)
												), 
												Arrays.asList(
													...
													new TableConditionEntry(TableNames.USERS, 
														Arrays.asList(
															new ConditionEntry(Arrays.asList(Operators.AND), "role", Arrays.asList(Operators.EQUAL), (short)1)
														)
													)
												)
											);
	...
	public static List<List<Model>> findOrganizations(Map<String, String> params, Map<String, String[]> queryParams) throws ResponseException {
		Dao orgDao = new Dao(OrganizationModel.class);
		...
		if(orgNames != null && orgNames.length > 0) {
			conditions.add(new ConditionEntry(Arrays.asList(operator), "organization_name", Arrays.asList(Operators.ILIKE), '%' + orgNames[0] + '%'));
			operator = Operators.AND;
		}
		
		String startedYears[] = queryParams.get("filter_startyear");
		if(startedYears != null && startedYears.length > 0) {
			conditions.add(new ConditionEntry(Arrays.asList(operator), "started_year", Arrays.asList(Operators.EQUAL), Short.parseShort(startedYears[0])));
			operator = Operators.AND;
		}
		...
		if(organizationStatuses != null && organizationStatuses.length > 0) {
			conditions.add(new ConditionEntry(Arrays.asList(operator), "organization_status", Arrays.asList(Operators.EQUAL), Short.parseShort(organizationStatuses[0])));
			operator = Operators.AND;
		}
		...
		List<List<Model>> orgDetailsList = orgDao.findAllWithJoin(
												...
												Arrays.asList(
														...
														new TableConditionEntry(TableNames.USERS, 
															Arrays.asList(
																new ConditionEntry(Arrays.asList(operator), "role", Arrays.asList(Operators.EQUAL), (short)1)
															)
														)
												), 
												limit, offset);
	...
	public static OrganizationModel updateOrganizationById(Map<String, String> params, OrganizationModel org) throws ResponseException {
		Dao orgDao = new Dao(OrganizationModel.class);
		List<Model> updatedOrgs = orgDao.updateAndReturn(org, Arrays.asList(new ConditionEntry(null, "organization_id", Arrays.asList(Operators.EQUAL), Long.parseLong(params.get("org_id")))), Arrays.asList("*"));
	...
	public static OrganizationModel deleteOrganizationById(Map<String, String> params) throws ResponseException {
		Dao orgDao = new Dao(OrganizationModel.class);
		List<Model> deletedOrgs = orgDao.deleteAndReturn(Arrays.asList(new ConditionEntry(null, "organization_id", Arrays.asList(Operators.EQUAL), Long.parseLong(params.get("org_id")))), Arrays.asList("*"));
